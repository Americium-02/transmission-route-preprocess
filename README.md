...
**求法（v0.7 起）：沿河道行走**，不再依赖全局主轴布站。

早期版本沿连通块主轴每 100m 作垂线取弦中点。这在顺直/缓弯河道上可行，但在**细窄蜿蜒**河道上会出错：垂直于全局轴的截线一次横穿好几个河湾，且沿轴等距 ≠ 沿河道等距（蛇形河弧长可达轴长 1.5 倍以上），相邻站在河道上隔两三百米，连线直接横切河湾。实测密采样出河率达 11~29%。

现改为局部行走：

| 环节 | 做法 |
|---|---|
| 起点 | 河段**中部**（端部是端帽区，截面被端边剪断，行走会在里面打转） |
| 起步方向 | 扫描 180° 取"垂直截面弦最短"者 = **局部**流向（蜿蜒河道中部的局部走向可能与全局主轴近乎垂直） |
| 每一步 | 在当前切向 ±75° 扇形内搜索可行方向 → 取垂直于该方向、含该点的弦 → 中点为新的中心点 |
| 步长 | **自适应** `clip(0.35×局部河宽, 4m, 0.5×interval)`；急弯处按上一步转角自动收步；落到面外则减半重试 |
| 守卫 | 横移 > 0.6×局部河宽 → 停；无前进 → 停；贴近多步以前的位置 → 停（防打转）。严格守卫走不通时先就地放宽重试，避免局部锯齿让整段提前终止 |
| 方向 | 从起点**正反两向**各走一遍再拼接 |

**择优闸门**：两种方法失效模式互补（轴扫描不会提前停但会横切河湾；行走忠实跟随但可能提前停）。故按**面积法**估出河段应有长度（`面积 ÷ 平均河宽`，对锯齿岸线稳健，周长法会被锯齿放大 2 倍以上），行走覆盖不足 75% 时再跑一次轴扫描，取**有效长度**（扣掉落在河面外的部分）更大者。

**端部处理**：布站不贴边；端点回折（钩子）与落在河面外的端点会被剪除。
